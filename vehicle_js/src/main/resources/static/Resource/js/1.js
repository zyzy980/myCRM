...
$(this).each(function(){
	var v = $(this).combobox('textbox');
	var arr = v.data("events")["focus"];
	var state = true;
	if(arr!=null && arr.length>0){
		for(var i=0;i<arr.length;i++){
			if(arr[i].namespace == "com2"){
				state=false;
				break;
			}			
		}
	...
	if(state){
		// 取个命名空间
		v.bind("focus.com2", function(){
			var _com = $(this).parent().prev();
			if(!_com.combobox("getValue")){
				_com.combobox("setText","");
			}
			_com.combobox('showPanel');
		});
		v.bind("blur.com2", function(){
			var _com = $(this).parent().prev();
			if(_com.combobox("getValue") == ""){
				_com.combobox("setValue","");
			}
		});
	}
