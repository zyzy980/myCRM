...
(function ($) {
    ...
    $.i18n.properties = function (settings) {
        ...
        /**
         * 如果已经加载过公共的，就不用重复去加载js了
         */

        var bool = false;
        for(var v in window.top.$.i18n.map){
            bool = true;
            break;
        }
        if(bool){
            $.i18n.map = JSON.parse(JSON.stringify(window.top.$.i18n.map));
        }else{
            var defaultFileName = settings.language+"/common";
        ...
        files.forEach(function (file) {
            ...
            var shortFileName = settings.path + file + '.properties';
...
function loadProperties() {
    ...
    $.i18n.properties({
        name: lang+"/"+formatFileName(),    //属性文件名     命名格式： 文件名_国家代号.properties
        path:path,   //注意这里路径是你属性文件的所在文件夹
        mode:'map',
        language: lang,//lang,     //这就是国家代号 name+language刚好组成属性文件名：strings+zh -> strings_zh.properties
        callback:function(){
            //data-locale="select"
            //setCommon(lang);
            $("label[data-locale]").each(function(){
                $(this).prop('outerHTML', $.i18n.prop($(this).attr("data-locale")));
                // $(this).html($.i18n.prop($(this).data("locale")));
            });
            $("*").each(function(){
                if(this.tagName != "label"){
                    for(var i = 0; i < this.attributes.length; i++){
                        if(/^data-locale-/.test(this.attributes[i].name)){
                            var newName = this.attributes[i].name.substring("data-locale-".length);
                            $(this).prop(newName, $.i18n.prop(this.attributes[i].value));
...
function formatFileName(prefix) {
    ...
    if(location.pathname == "/"){
        ...
        pathname = location.pathname.substring(location.pathname.substring(1).indexOf("/")+1);
...
/*
    ...
    var result = prefix;
    pathname = pathname.substring(pathname.indexOf("View/") + 5, pathname.indexOf('.'));
...
function setCommon(lang) {
    $.ajax({
        url: '../../Resource/i18n/common_'+ lang +'.properties',
        async: false,
        dataType: 'text',
        success: function (data, status) {
            var coommon = JSON.parse(data.toString());
            for(var key in coommon){
                $.i18n.map[key] = coommon[key];
            }
        },
        error: function (jqXHR, textStatus, errorThrown) {
            console.log("拉取通用文件失败")
        }
    });
